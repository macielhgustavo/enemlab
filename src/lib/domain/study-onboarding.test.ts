...
describe("study onboarding", () => {
  ...
  it("não interrompe ao trocar de provider depois da primeira configuração", () => {
    const db = makeDB({ activeProvider: "ita" });
    setProviderStudyConfig(db, "enem", {
      objective: "balanced",
      onboardingCompletedAt: "2026-09-22T12:00:00.000Z",
    });

    expect(needsStudyOnboarding(db, "ita")).toBe(false);
  });
