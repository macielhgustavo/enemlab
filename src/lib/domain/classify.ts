 ...
 * questões reference-only, onde o texto não pode ser redistribuído. Quando um
 * overlay visual precisou ocultar o texto, reutilizamos o snapshot calculado
 * sobre a questão original. Só depois disso cai no classificador textual v7.2.
 */
export function classifyQuestion(q: Question): base.QuestionClassification {
  ...
  if (q.classificationSnapshot) return q.classificationSnapshot;
  return base.classifyQuestion(q);
}
