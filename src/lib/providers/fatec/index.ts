...
export function fatecEditions() {
  return EDITIONS;
}

export function fatecYears(): number[] {
  return [...new Set(EDITIONS.map((edition) => edition.year))].sort((a, b) => b - a);
}

export function fatecAnswerKey(editionId: string): ReferenceAnswerKey | null {
  return CATALOG[editionId] ?? null;
}

export function fatecExamUrl(editionId: string): string | null {
  return fatecAnswerKey(editionId)?.examUrl ?? null;
}

export function fatecQuestions(editionId: string): NormalizedQuestion[] {
  const key = fatecAnswerKey(editionId);
  return key ? referenceQuestionsForKey(fatecProviderConfig, key) : [];
}

export function fatecQuestionKey(question: NormalizedQuestion): string {
  return referenceQuestionKey(FATEC_PROVIDER_ID, question);
}

export function fatecMeasure(editionId: string) {
  const key = fatecAnswerKey(editionId);
