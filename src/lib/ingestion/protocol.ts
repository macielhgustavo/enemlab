...
function parseDocuments(value: unknown): ExternalExtractionDocument[] {
  if (!Array.isArray(value)) {
    throw new Error("external extraction: documents must be an array");
  }
  return value.map((document, index) => {
    ...
    const sha256 = requireString(
      document.sha256,
      `documents[${index}].sha256`,
    ).toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(sha256)) {
      throw new Error(
        `external extraction: documents[${index}].sha256 must be SHA-256`,
      );
    }
    return { url, sha256 };
  });
...
function parseFallbackTarget(
  value: unknown,
  index: number,
): SelectiveFallbackTarget {
  if (!isRecord(value)) {
    throw new Error(
      `external extraction: fallbackRequest.targets[${index}] must be an object`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `external extraction: fallbackRequest.targets[${index}].reason is invalid`,
    );
  }
  const questionNumber =
    value.questionNumber === undefined
      ...
      : requireInteger(
          ...
          `fallbackRequest.targets[${index}].questionNumber`,
        );
  const page =
  ...
  if (
    (questionNumber !== undefined && questionNumber < 1) ||
    (page !== undefined && page < 1)
  ) {
    throw new Error(
      `external extraction: fallbackRequest.targets[${index}] locator must be positive`,
    );
  }
  if (questionNumber === undefined && page === undefined) {
    throw new Error(
      `external extraction: fallbackRequest.targets[${index}] needs questionNumber or page`,
    );
  }
  return {
    questionNumber,
    ...
    message: requireString(
      value.message,
      `fallbackRequest.targets[${index}].message`,
    ),
  };
}
...
function parseFallbackRequest(value: unknown): SelectiveFallbackRequest {
  if (!isRecord(value) || !Array.isArray(value.targets) || value.targets.length === 0) {
    throw new Error(
      "external extraction: fallbackRequest.targets must be a non-empty array",
    );
  }
  return { targets: value.targets.map(parseFallbackTarget) };
}

function parseEnvelope(value: unknown): ExternalExtractionEnvelope {
  if (!isRecord(value)) {
    throw new Error("external extraction: payload must be an object");
  }
  if (value.protocolVersion !== EXTRACTION_PROTOCOL_VERSION) {
    throw new Error(
      `external extraction: unsupported protocolVersion ${String(value.protocolVersion)}`,
    );
  }
  if (!isRecord(value.extraction)) {
    throw new Error("external extraction: extraction is required");
  }
  ...
  if (
    ...
    throw new Error(
      "external extraction: extraction.semanticFidelityIssues must be an array",
    );
  }
...
function parseFailureEnvelope(value: unknown): ExternalExtractionFailureEnvelope {
  ...
  if (!isRecord(value.error)) {
    throw new Error("external extraction: error is required");
  }
...
/**
 * One physical PDF may appear more than once in an ingestion context when it
 * fulfills multiple logical roles. External protocols bind to physical bytes,
 * so those logical aliases collapse to one URL/SHA pair.
 */
function physicalContextDocuments(
  context: IngestionExtractionContext,
): ExternalExtractionDocument[] {
  const byUrl = new Map<string, string>();
  for (const { fingerprint } of context.documents) {
    const sha256 = fingerprint.sha256.toLowerCase();
    const previous = byUrl.get(fingerprint.url);
    if (previous && previous !== sha256) {
      throw new Error(
        `external extraction: conflicting SHA-256 for duplicate document ${fingerprint.url}`,
      );
    }
    byUrl.set(fingerprint.url, sha256);
  }
  return [...byUrl.entries()].map(([url, sha256]) => ({ url, sha256 }));
}
...
function verifyEnvelopeBinding(
  ...
  const expectedDocuments = new Map(
    physicalContextDocuments(context).map((document) => [
      document.url,
      document.sha256,
    ]),
  );
  if (envelope.documents.length !== expectedDocuments.size) {
  ...
  for (const document of envelope.documents) {
    ...
    if (expectedSha !== document.sha256) {
      throw new Error(
        `external extraction: stale document fingerprint for ${document.url}`,
      );
    }
  }
}
 ...
 * extractor only when it is cryptographically bound to the exact physical
 * documents downloaded by this engine job.
 */
export function consumeExternalExtraction(
  ...
  if (
    isRecord(payload) &&
    payload.protocolVersion === EXTRACTION_FAILURE_PROTOCOL_VERSION
  ) {
  ...
  return {
    ...
    annulled: envelope.extraction.annulled
      ? [...envelope.extraction.annulled]
      : undefined,
    subjects: envelope.extraction.subjects
      ? { ...envelope.extraction.subjects }
      : undefined,
...
export function createExternalExtractionEnvelope(
  ...
  return {
    ...
    extractor: { ...extractor },
    documents: physicalContextDocuments(context),
    extraction,
  };
}
...
export function createExternalExtractionFailureEnvelope(
  ...
  if (!fallbackRequest.targets.length) {
    throw new Error(
      "external extraction failure envelope requires fallback targets",
    );
  }
  return {
    protocolVersion: EXTRACTION_FAILURE_PROTOCOL_VERSION,
